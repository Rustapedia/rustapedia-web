          ...
          {currentItem.gatheringInfo.map(items => (
            <tr key={items.id}>
              <td className="tableCell">
                <StyledLink
                  to={removeSpace(items.tool.name)}
                  key={items.tool.id}
                >
              ...
              <td className="tableCell center">
                {items.result.map(res => (
                  <span key={res.id}>
                    <Link to={removeSpace(res.item.name)} key={res.item.id}>
                      {res.item.image !== null && (
                        <Img
        ...
        <tbody>
          ...
          {currentItem.gather.map(items => (
            <tr key={items.id}>
              <td className="tableCell">
                <StyledLink
                  key={items.fromItem.id}
                  to={removeSpace(items.fromItem.name)}
                >
      ...
      {currentItem.gatheredFrom.length > 0 && (
        <tbody>
          <tr className="center">
            ...
            {toolArray.map(items => (
              <th key={items.id}>
                <Link to={removeSpace(items.name)}>
                  {items.image !== null && (
                    <Img
                      key={items.image.id}
                      className="ingredients"
          ...
          {currentItem.gatheredFrom.map(items => (
            <tr key={items.id}>
              <td className="tableCell">
                <StyledLink to={removeSpace(items.name)}>
                  {items.image !== null && (
                    <Img
                      key={items.image.id}
                      className="ingredients"
                ...
                return itemFound !== undefined ? (
                  <td className="tableCell center" key={itemFound.tool.id}>
                    {itemFound.result.map(res => (
                      <tr key={res.id}>
                        <Link to={removeSpace(res.item.name)}>
                          {res.item.image !== null && (
                            <Img
                              key={res.item.image.id}
                              className="ingredients"
